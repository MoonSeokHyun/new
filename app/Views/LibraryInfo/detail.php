<?php
$libName = esc($library['Library Name'] ?? '도서관');
    ...
    "@graph":[
      ...
      },
      {
        ...
        "itemListElement":[
          ...
          {"@type":"ListItem","position":2,"name":"도서관 목록","item":"<?= esc($librariesUrl) ?>"},
          {"@type":"ListItem","position":3,"name":"<?= esc($district_name) ?>","item":"<?= esc($districtUrl) ?>"},
          {"@type":"ListItem","position":4,"name":"<?= esc($libName) ?>","item":"<?= esc($canonicalUrl) ?>"}
        ]
      },
      {
        "@type":"Library",
        "@id":"<?= esc($canonicalUrl) ?>#library",
        "name":"<?= esc($libName) ?>",
        "url":"<?= esc($canonicalUrl) ?>",
        "telephone":"<?= esc($phone) ?>",
        "address":{
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="kv">
        ...
        <?php if ($library['Library Type'] ?? ''): ?><span class="pill"><?= esc($library['Library Type']) ?></span><?php endif; ?>
      ...
      <div class="actions">
        ...
        <a class="btn" href="<?= $districtUrl ?>">같은 지역 도서관</a>
        <a class="btn" href="<?= $librariesUrl ?>">도서관 목록</a>
      </div>
    </div>
    <div class="ad">
    ...
    <div class="card">
      <h2>기본 정보</h2>
      <ul class="detail">
        <li class="row"><span class="label">도서관명</span><span class="value"><?= esc($libName) ?></span></li>
        <li class="row"><span class="label">도로명주소</span><span class="value"><?= $road_address ?></span></li>
        <li class="row"><span class="label">시도</span><span class="value"><?= esc($library['Province/City'] ?? '') ?></span></li>
        <li class="row"><span class="label">시군구</span><span class="value"><?= esc($library['City/County/District'] ?? '') ?></span></li>
        <li class="row"><span class="label">도서관 유형</span><span class="value"><?= esc($library['Library Type'] ?? '') ?></span></li>
        <li class="row"><span class="label">운영기관</span><span class="value"><?= esc($library['Operating Organization'] ?? '') ?></span></li>
        <li class="row"><span class="label">전화번호</span><span class="value"><?= $phone ?></span></li>
        <li class="row"><span class="label">웹사이트</span><span class="value"><?php if ($library['Website URL'] ?? ''): ?><a href="<?= esc($library['Website URL']) ?>" target="_blank" rel="nofollow"><?= esc($library['Website URL']) ?></a><?php endif; ?></span></li>
        <li class="row"><span class="label">휴관일</span><span class="value"><?= esc($library['Closed Days'] ?? '') ?></span></li>
      </ul>
    </div>
    <div class="card">
      <h2>운영 시간</h2>
      <ul class="detail">
        <li class="row"><span class="label">평일 개방</span><span class="value"><?= esc($library['Weekday Opening Time'] ?? '') ?></span></li>
        <li class="row"><span class="label">평일 폐장</span><span class="value"><?= esc($library['Weekday Closing Time'] ?? '') ?></span></li>
        <li class="row"><span class="label">토요일 개방</span><span class="value"><?= esc($library['Saturday Opening Time'] ?? '') ?></span></li>
        <li class="row"><span class="label">토요일 폐장</span><span class="value"><?= esc($library['Saturday Closing Time'] ?? '') ?></span></li>
        <li class="row"><span class="label">공휴일 개방</span><span class="value"><?= esc($library['Holiday Opening Time'] ?? '') ?></span></li>
        <li class="row"><span class="label">공휴일 폐장</span><span class="value"><?= esc($library['Holiday Closing Time'] ?? '') ?></span></li>
      </ul>
    </div>
    <div class="card">
      <h2>시설 정보</h2>
      <ul class="detail">
        <li class="row"><span class="label">열람 좌석 수</span><span class="value"><?= esc($library['Number of Reading Seats'] ?? '') ?></span></li>
        <li class="row"><span class="label">도서 수</span><span class="value"><?= esc($library['Number of Materials (Books)'] ?? '') ?></span></li>
        <li class="row"><span class="label">정기 간행물 수</span><span class="value"><?= esc($library['Number of Materials (Serials)'] ?? '') ?></span></li>
        <li class="row"><span class="label">비도서 자료 수</span><span class="value"><?= esc($library['Number of Materials (Non-Book)'] ?? '') ?></span></li>
        <li class="row"><span class="label">대출 가능 항목 수</span><span class="value"><?= esc($library['Number of Lending Items Allowed'] ?? '') ?></span></li>
        <li class="row"><span class="label">대출 기간 (일)</span><span class="value"><?= esc($library['Lending Period (Days)'] ?? '') ?></span></li>
        <li class="row"><span class="label">토지 면적</span><span class="value"><?= esc($library['Land Area'] ?? '') ?></span></li>
        <li class="row"><span class="label">건물 면적</span><span class="value"><?= esc($library['Building Area'] ?? '') ?></span></li>
      </ul>
    ...
    <div class="card" id="nearbySection">
      <h2>근처 도서관 보기</h2>
      <?php if (!empty($nearby_libraries)): ?>
        <div class="near-grid">
          <?php foreach ($nearby_libraries as $n): ?>
            <?php
              $nName = esc($n['Library Name'] ?? '도서관');
              $nUrl  = esc($n['url'] ?? '#');
              $nRoad = esc($n['Address (Road Name)'] ?? '');
              $addr  = $nRoad;
            ?>
            <div class="near-item">
      ...
      <?php else: ?>
        <p class="note">
          가까운 도서관을 찾지 못했습니다. <a href="<?= $districtUrl ?>"><?= esc($district_name) ?> 도서관 목록</a>에서 더 찾아보세요.
        </p>
      <?php endif; ?>
    </div>
    <div class="ad">
