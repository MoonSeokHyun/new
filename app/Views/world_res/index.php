...
<div class="container">
  ...
  <div class="top">
    <div>
      <h1><?= esc($search !== '' ? "“{$search}” 검색 결과" : "세계 음식점 목록") ?></h1>
      <p>주소/영업시간/편의시설 정보를 빠르게 확인하고 상세 페이지에서 지도와 근처 음식점도 보세요.</p>
    </div>

    <form class="search" method="get" action="<?= esc($listUrl) ?>">
      <input type="text" name="search" value="<?= esc($search) ?>" placeholder="음식점명/주소로 검색" />
    ...
    <?php if (!empty($restaurants) && is_array($restaurants)): ?>
      ...
      ?>
        <?php
          ...
          $name  = esc($res['FCLTY_NM'] ?? '음식점');
          $addr  = esc($res['RDNMADR_NM'] ?? ($res['LNM_ADDR'] ?? ''));
          $phone = esc($res['TEL_NO'] ?? '');
          $cat1  = esc($res['CTGRY_ONE_NM'] ?? '');
          $url   = site_url('world-res/detail/' . $id);
        ?>
        <a class="card" href="<?= esc($url) ?>">
          ...
          <div class="chips">
            <?php if ($cat1): ?><span class="chip"><?= $cat1 ?></span><?php endif; ?>
            <?php if ($res['CTGRY_TWO_NM'] ?? ''): ?>
              <span class="chip"><?= esc($res['CTGRY_TWO_NM']) ?></span>
            <?php endif; ?>
          </div>
        </a>
  ...
  <div class="pager-wrap">
    <?php if (isset($pager) && $pager): ?>
      <?= $pager->links('restaurants', 'default_full') ?>
    <?php endif; ?>
  </div>
