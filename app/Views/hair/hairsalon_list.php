...
<div class="container">
  ...
  <div class="top">
    <div>
      <h1><?= esc($search !== '' ? "“{$search}” 검색 결과" : "미용실 목록") ?></h1>
      <p>주소/전화/업종 정보를 빠르게 확인하고 상세 페이지에서 지도와 근처 미용실도 보세요.</p>
    </div>

    <form class="search" method="get" action="<?= esc($listUrl) ?>">
      <input type="text" name="search" value="<?= esc($search) ?>" placeholder="상호/주소로 검색" />
    ...
    <?php if (!empty($salons) && is_array($salons)): ?>
      ...
      ?>
        <?php
          ...
          $name  = esc($salon['business_name'] ?? '미용실');
          $addr  = esc($salon['road_name_address'] ?? ($salon['full_address'] ?? ''));
          $phone = esc($salon['contact_phone_number'] ?? '');
          $type  = esc($salon['business_type_name'] ?? '');
          $url   = site_url('hairsalon/detail/' . $id);
        ?>
        <a class="card" href="<?= esc($url) ?>">
          ...
          <div class="chips">
            ...
            <?php if (($salon['business_status_name'] ?? '') !== ''): ?>
              <span class="chip">영업: <?= esc($salon['business_status_name']) ?></span>
            <?php endif; ?>
          </div>
        </a>
