...
$seoTitle = "{$facilityName} | {$district_name} 체육시설 위치·면적·수용인원";
$seoDescription = "{$district_name}에 위치한 {$facilityName} 체육시설 정보. {$road_address} 위치, 면적, 수용인원, 담당부서를 확인하고 네이버 지도로 위치도 바로 확인하세요.";
    ...
    "@graph":[
      ...
      },
      {
        ...
        "itemListElement":[
          ...
          {"@type":"ListItem","position":2,"name":"체육시설 목록","item":"<?= esc($facilitiesUrl) ?>"},
          {"@type":"ListItem","position":3,"name":"<?= esc($district_name) ?>","item":"<?= esc($districtUrl) ?>"},
          {"@type":"ListItem","position":4,"name":"<?= esc($facilityName) ?>","item":"<?= esc($canonicalUrl) ?>"}
        ]
      },
      {
        "@type":"SportsActivityLocation",
        "@id":"<?= esc($canonicalUrl) ?>#facility",
...
<div class="container">
  <div class="breadcrumb">
    <a href="<?= site_url() ?>">홈</a> &gt;
    <a href="<?= $facilitiesUrl ?>">체육시설 목록</a> &gt;
    <a href="<?= $districtUrl ?>"><?= esc($district_name) ?></a> &gt;
    상세정보
  </div>
  <h1 class="title"><?= esc($facilityName) ?></h1>
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="kv">
        ...
        <?php if ($facility['FCLTY_TY_NM'] ?? ''): ?><span class="pill"><?= esc($facility['FCLTY_TY_NM']) ?></span><?php endif; ?>
      ...
      <div class="actions">
        ...
        <a class="btn" href="<?= $districtUrl ?>">같은 지역 체육시설</a>
        <a class="btn" href="<?= $facilitiesUrl ?>">체육시설 목록</a>
      </div>
    </div>
    <div class="ad">
    ...
    <div class="card">
      <h2>기본 정보</h2>
      <ul class="detail">
        <li class="row"><span class="label">시설명</span><span class="value"><?= esc($facilityName) ?></span></li>
        <li class="row"><span class="label">도로명주소</span><span class="value"><?= $road_address ?></span></li>
        <li class="row"><span class="label">시설 유형</span><span class="value"><?= esc($facility['FCLTY_TY_NM'] ?? '') ?></span></li>
        <li class="row"><span class="label">업종명</span><span class="value"><?= esc($facility['INDUTY_NM'] ?? '') ?></span></li>
        <li class="row"><span class="label">소유주체</span><span class="value"><?= esc($facility['POSESN_MBY_NM'] ?? '') ?></span></li>
        <li class="row"><span class="label">담당부서</span><span class="value"><?= esc($facility['RSPNSBLTY_DEPT_NM'] ?? '') ?></span></li>
        <li class="row"><span class="label">전화번호</span><span class="value"><?= $phone ?></span></li>
        <li class="row"><span class="label">홈페이지</span><span class="value"><?php if ($facility['FCLTY_HMPG_URL'] ?? ''): ?><a href="<?= esc($facility['FCLTY_HMPG_URL']) ?>" target="_blank" rel="nofollow"><?= esc($facility['FCLTY_HMPG_URL']) ?></a><?php endif; ?></span></li>
      </ul>
    </div>
    <div class="card">
      <h2>시설 정보</h2>
      <ul class="detail">
        <li class="row"><span class="label">면적 (㎡)</span><span class="value"><?= esc($facility['FCLTY_AR_CO'] ?? '') ?></span></li>
        <li class="row"><span class="label">수용인원</span><span class="value"><?= esc($facility['ACMD_NMPR_CO'] ?? '') ?></span></li>
        <li class="row"><span class="label">장애인편의수</span><span class="value"><?= esc($facility['ADTM_CO'] ?? '') ?></span></li>
        <li class="row"><span class="label">보조시설 여부</span><span class="value"><?= esc($facility['NATION_ALSFC_AT'] ?? '') ?></span></li>
      </ul>
    ...
    <div class="card" id="nearbySection">
      <h2>근처 체육시설 보기</h2>
      <?php if (!empty($nearby_facilities)): ?>
        <div class="near-grid">
          <?php foreach ($nearby_facilities as $n): ?>
            <?php
              $nName = esc($n['FCLTY_NM'] ?? '체육시설');
              $nUrl  = esc($n['url'] ?? '#');
      ...
      <?php else: ?>
        <p class="note">
          가까운 체육시설을 찾지 못했습니다. <a href="<?= $districtUrl ?>"><?= esc($district_name) ?> 체육시설 목록</a>에서 더 찾아보세요.
        </p>
      <?php endif; ?>
    </div>
    <div class="ad">
